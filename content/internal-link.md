...
內鏈對 SEO 很重要，一方面讓 Google 更容易爬取站內頁面，另一方面能把流量在站內導流，讓讀者從一篇文章連到相關文章或產品頁面。

問題是，站內文章一多，要人工去找「這篇文章應該連到哪些頁面」根本做不完。你需要知道站內有哪些文章、哪些產品、哪些關鍵字跟當前文章相關，全靠人腦記憶和搜尋效率非常低。
...
**第一塊，找出值得優化的文章。** 先接 GA4 和 Google Search Console 的 API，撈出最近一段時間的文章流量、點擊、曝光、排名數據。再依據這些指標的組合，把文章分成幾種類型，例如，明星文章（全面表現優秀）、高潛力（曝光高但 CTR 低）、待提升（排名在 10-30 名）、隱藏寶石（GA4 流量高但搜尋曝光少）等，讓你知道要從哪些文章開始優先補內鏈、以及每種類型對應的優化方向。

**第二塊，分析文章缺少哪些連結機會。** 選定一篇文章後，工具會用 Playwright 爬取文章內容，同時記錄文章裡現有的內部連結。接著把文章丟給 LLM，提取具有語意關聯的關鍵詞，分成產品、品牌、技術規格、延伸主題四類，並給出信心度分數。

**第三塊，推薦可以連結的站內頁面。** 根據提取到的關鍵詞，搜尋站內相關文章和產品頁面，最後生成一份連結建議清單，包括建議連結的位置（錨文字）、對應的目標頁面，以及推薦原因。
...
這個工具在動工前，目錄是完全空的。我先用 Claude 做了一輪完整的架構設計，把功能需求、內容平台頁面結構、技術考量全部整理成一份設計文件，讓 AI 輸出建議的技術棧、模組切分、API 設計、資料流程，然後再整批交給 Claude Code 實作。
...
爬蟲部分，選 Playwright 而非 cheerio 等靜態 parser，是因為我所在公司的內容平台頁面有動態渲染的部分，直接抓 HTML 拿不到完整內容。

但 Playwright 在不同頁面結構下的穩定性需要反覆調整。早期版本遇到的問題是，某些頁面會先回傳骨架 HTML、內容再非同步填入，`page.waitForSelector` 可以抓到元素，但文字還沒進來。後來改用 `waitUntil: 'domcontentloaded'` 搭配 `waitForSelector` 的雙重等待策略，才把這個問題消掉。

加上 SQLite 快取（24 小時 TTL）之後，重複分析同一篇文章的速度才變得可以接受，不用每次都重新跑一遍 Playwright。

爬蟲本身也加了請求節流，批次爬多篇文章時每次呼叫之間固定等待約 1 秒，讓請求不會瞬間打爆對方伺服器。

## 技術選擇

幾個關鍵技術選擇值得記錄一下。

**LLM 呼叫用 OpenRouter** 而不是直接打某家廠商 API。好處是可以彈性切換模型，不被單一服務商綁住。預設用 Claude 3 Haiku，成本低、速度快，適合關鍵詞提取這種「需要大量呼叫、但不需要最強模型」的場景。如果哪天需要更高品質的分析，只要改一個 model 字串就能換模型，不用動其他程式碼。
...
**Playwright 做爬蟲** 而不是 cheerio 等靜態 parser。內容平台部分頁面是動態渲染，需要執行 JavaScript 才能拿到完整內容，所以選了 Playwright。代價是速度比靜態 parser 慢，這也是為什麼快取機制不可或缺。
