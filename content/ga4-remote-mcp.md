...
**Senao FrontAPI**，公開的電商站內搜尋 API，涵蓋商品搜尋、autocomplete、文章搜尋、單篇文章抓取。
...
**第五階段（2026-04-10）**，先降低 Supabase auth store 的 DB 壓力（每次請求都 prune 太頻繁），然後接入 Senao 的公開 FrontAPI，server 正式從 Google analytics bridge 變成混合型商業分析 MCP。
...
### Senao FrontAPI 是逆向工程出來的

加入電商站內搜尋這個需求，一開始打算用 HTML 爬蟲，但很快就發現太脆。後來在 Claude Code 幫忙下，直接翻站台載入的 JavaScript 檔案，找到 `/FrontAPI/Search`（POST）、`/FrontAPI/Search/autoComplete`（GET）、`/FrontAPI/Article/getArticles`（GET）這三個 JSON API，全部都是公開不需要 auth 的端點，只要帶正確的 `application/x-www-form-urlencoded` body 就能拿到結構化資料。
...
做 Senao 電商站分析時，逐漸釐清了一個具體的分析需求，文章頁面沒有自訂埋碼，唯一能做文章到商品歸因的方式，是文章內的商品連結加 `?content={articleId}` query param，透過 `pagePathPlusQueryString` 來追蹤。這個限制反過來推動了 `ga4_run_funnel_report` 的 open funnel 支援，以及 `funnelBreakdown` 讓文章成效可以依路徑拆行，還有對 `pagePathPlusQueryString` 這類維度的明確支援，而不只是 `pagePath`（Funnel API 不接受 `pagePath`，只接受 `pageLocation` 或 `pagePathPlusQueryString`，這個限制直到踩到錯才發現）。
...
這個 repo 最初只是想把 GA4 接進 Claude，最後卻因為「要讓它在真正的服務上活著」，花了相當多力氣在 OAuth 設計、store 韌性、雙模式支援上。把 Senao FrontAPI 接進來那段，也算是意外的收穫，從爬蟲思路轉到找 JSON API，讓整個整合乾淨很多。
